...
def search_cpf():
    ...
    if re.match(r'^\d{2}\.\d{2}\.\d+-\d$', raw_cpf):
        try:
            with engine.connect() as conn:
                q_rev = text("SELECT cpf FROM app_vendas.SALES_CAD_UNICO_JSON WHERE cartoes_json LIKE :card LIMIT 1")
    ...
    try:
        with engine.connect() as conn:
            ...
            q1 = text("""
                ...
                FROM 
                    app_cliente.customer c 
                LEFT JOIN 
                    app_cliente.address a ON a.id_customer = c.id
            ...
            # 4. app_vendas.WIFIMAX_USERS
            q4 = text("SELECT MAX(EMAIL) as email, MAX(TELEFONE) as celular FROM app_vendas.WIFIMAX_USERS WHERE CPF = :cpf GROUP BY CPF")
            row4 = conn.execute(q4, {"cpf": cpf_limpo}).fetchone()
            if row4: result['wifi_users'] = {'email': row4[0], 'celular': row4[1]}

            # 5. app_vendas.CLIENTES_WHATSAPP
            q5 = text("SELECT telefone FROM app_vendas.CLIENTES_WHATSAPP WHERE cpf = :cpf LIMIT 1")
            row5 = conn.execute(q5, {"cpf": cpf_limpo}).fetchone()
            if row5: result['whatsapp'] = {'telefone': row5[0]}
            
            # 6. app_vendas.SALES_CAD_UNICO_JSON
            q6 = text("SELECT * FROM app_vendas.SALES_CAD_UNICO_JSON WHERE cpf = :cpf LIMIT 1")
