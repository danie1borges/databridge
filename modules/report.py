...
_NETSALES_LAST_SALE_SQL = """(
    SELECT MAX(n.DATA)
    FROM app_vendas.COM_NETSALES n
...
_REPORT_LIVE_SOURCE_SQL_TEMPLATE = """
    ...
    FROM app_vendas.SALES_CAD_UNICO_JSON j
...
def augment_results_with_contacts(flat_rows, conn):
    ...
    legacydb_q = f"SELECT REPLACE(REPLACE(cpf, '.', ''), '-', '') as num_cpf, nome, email, telefone as cellphone FROM app_vendas.SALES_CAD_UNICO_JSON WHERE cpf IN ({cpf_placeholders})"
    ...
    wifi_q = f"SELECT CPF as num_cpf, MAX(EMAIL) as email, MAX(TELEFONE) as cellphone FROM app_vendas.WIFIMAX_USERS WHERE CPF IN ({cpf_placeholders}) GROUP BY CPF"
    wifis = {r['num_cpf']: dict(r) for r in conn.execute(text(wifi_q), cpf_params).mappings().fetchall()}

    whatsapp_q = f"SELECT DISTINCT cpf as num_cpf, telefone as cellphone FROM app_vendas.CLIENTES_WHATSAPP WHERE cpf IN ({cpf_placeholders})"
        ...
        if field_id == 'aprovado_sem_cartao_estudantil':
            sql_parts.append("""
                (
                    ...
                    AND NOT EXISTS (
                        SELECT 1
                        FROM app_vendas.SALES_CAD_UNICO_JSON j2
