...
def refresh_dashboard_cache():
    ...
    try:
        with engine.connect() as conn:
            stats['cad_unico'] = {
                'total': conn.execute(text("SELECT COUNT(*) FROM app_vendas.SALES_CAD_UNICO_JSON")).scalar(),
                'sem_email': conn.execute(text("SELECT COUNT(*) FROM app_vendas.SALES_CAD_UNICO_JSON WHERE email IS NULL OR email = ''")).scalar(),
                'sem_celular': conn.execute(text("SELECT COUNT(*) FROM app_vendas.SALES_CAD_UNICO_JSON WHERE telefone IS NULL OR telefone = ''")).scalar(),
                'sem_endereco': conn.execute(text("SELECT COUNT(*) FROM app_vendas.SALES_CAD_UNICO_JSON WHERE endereco IS NULL OR endereco = ''")).scalar()
            }
            q_cliente = text(
            ...
            q_abt = text("""
                SELECT 
                    ...
                    SUM(CASE WHEN email IS NULL OR email = '' THEN 1 ELSE 0 END) as sem_email,
                    SUM(CASE WHEN celular IS NULL OR celular = '' THEN 1 ELSE 0 END) as sem_celular 
                FROM app_vendas.COM_CLIENTES_ABT
            """)
            ...
            q_wifi = text("SELECT COUNT(DISTINCT CPF) as total, SUM(CASE WHEN EMAIL IS NULL OR EMAIL = '' THEN 1 ELSE 0 END) as sem_email, SUM(CASE WHEN TELEFONE IS NULL OR TELEFONE = '' THEN 1 ELSE 0 END) as sem_celular FROM app_vendas.WIFIMAX_USERS")
            ...
            total_whatsapp = conn.execute(text("SELECT COUNT(DISTINCT cpf) FROM app_vendas.CLIENTES_WHATSAPP")).scalar()
            stats['whatsapp'] = {
...
def build_attention_payload(include_sensitive=False):
    with engine.connect() as conn:
        q = text("""
            ...
            FROM app_vendas.SALES_CAD_UNICO_JSON scuj
            WHERE
    ...
    try:
        with engine.connect() as conn:
            sem_legacydb_count = conn.execute(text("""
                ...
                LEFT JOIN app_vendas.SALES_CAD_UNICO_JSON scuj
            ...
            sem_cartao_count = conn.execute(text("""
                SELECT COUNT(DISTINCT REGEXP_REPLACE(va.cpf, '[^0-9]', ''))
                FROM databridge_db.vw_alunos_aprovados va
                INNER JOIN app_vendas.SALES_CAD_UNICO_JSON scuj
                    ON REGEXP_REPLACE(va.cpf, '[^0-9]', '') =
                       REPLACE(REPLACE(CONVERT(scuj.cpf USING utf8mb4) COLLATE utf8mb4_unicode_ci, '.', ''), '-', '')
                WHERE va.status = 'Aprovado'
                  AND (
